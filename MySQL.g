...
;
table_atom:
	  ( table_spec (partition_clause)? (alias)? (index_hint_list)? ) -> ^(table_spec)
	| ( subquery alias ) -> ^(subquery)
	| ( LPAREN table_references RPAREN ) -> ^(table_references)
	| ( OJ_SYM table_reference LEFT OUTER JOIN_SYM table_reference ON expression ) -> ^(JOIN_SYM table_reference table_reference)
;
join_condition:
...
select_expression:
	...
	( 
		FROM table_references ( -> ^(SELECT table_references))
		( partition_clause )?
		( where_clause )? 
		( groupby_clause )?
		( having_clause )?
...
subquery:
	LPAREN select_statement RPAREN -> ^(select_statement)
;

table_spec:
	( schema_name DOT )? table_name -> ^(TABLE_REF schema_name table_name)
;
...
delete_statements:
	...
	( delete_single_table_statement -> ^(DELETE_SYM delete_single_table_statement) | delete_multiple_table_statement1 -> ^(DELETE_SYM delete_multiple_table_statement1) | delete_multiple_table_statement2 -> ^(DELETE_SYM delete_multiple_table_statement2) )
;
